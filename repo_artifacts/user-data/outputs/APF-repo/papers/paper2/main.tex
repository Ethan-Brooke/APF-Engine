...
Section~\ref{sec:gauge_template} is the main event.  The three carrier requirements meet the complete classification of compact simple Lie algebras---all 17 of them---and every candidate except one is eliminated.  $\SU(3) \times \SU(2) \times \U(1)$ is not selected from a shortlist; it is the only structure that survives.  An exhaustive scan of 1{,}680 chiral fermion templates under seven simultaneous filters then yields a single survivor: the Standard Model at 45 Weyl fermions.  A final subsection derives the scalar sector: one complex $\SU(2)$ doublet, four real components, derived from the chiral fermion content and A1 minimality.
...
\textbf{Statement.}  The local enforcement algebra acts irreducibly on the GNS Hilbert space $\mathcal{H}_\omega$ (Theorem~$T_{\mathrm{GNS}}$, Formal Supplement~\cite{BrookeSupplement}).  That is, $\mathcal{H}_\omega$ has no proper closed subspace that is invariant under every enforcement operator.
...
\smallskip\noindent\textit{Logical status.}  Irreducibility is a \emph{consequence} of the derived algebraic structure, not a premise in the derivation of noncommutativity.  The primary noncommutativity proof ($T_{\mathrm{alg}}$, Formal Supplement) is pre-Hilbert: it derives $[E_{d_1}, F_\Pi] \ne 0$ from $L_\Delta$, $L_{\mathrm{blk}}$, and $L_\Pi$ without invoking Hilbert space or Schur's lemma.  $L_{\mathrm{irred}}$ then confirms that the algebra, once represented on $\mathcal{H}_\omega$, is irreducible---a consistency check, not a load-bearing step.
...
\smallskip\noindent\textbf{Proof (primary route: pre-Hilbert, from Formal Supplement~\cite{BrookeSupplement}).}  Three steps.
...
\item \textit{Source}: $L_{\mathrm{nc}}$ (non-closure requires stable composites) $+$ $T_{\mathrm{confinement}}$ (IR saturation makes non-singlets inadmissible; schematic $\beta$-function sign: Formal Supplement~\cite{BrookeP2Supplement}, \S4) $+$ $T_M$ (enforcement independence requires oriented composites) $+$ $B_1'$ (complex type blocks $B \leftrightarrow \bar{B}$ identification; Formal Supplement~\cite{BrookeP2Supplement}, \S3.3).
\end{varlist}
...
\smallskip\noindent\textit{Why gauge constraints are not subtracted (the BRST question).}\quad A field-theorist's instinct is to subtract gauge redundancies via BRST ghost counting, reducing the number of physical degrees of freedom below $\dim(G)$.  The admissibility framework does not subtract because the counting principle is different: what A1 constrains is the number of \emph{independently enforceable structural distinctions}, not the number of propagating modes.  Each Lie-algebra direction is a structural enforcement channel that the interface must independently track, regardless of whether its kinematic content propagates as a physical particle or is removed by gauge-fixing.  The Formal Supplement~\cite{BrookeP2Supplement}, \S III.1.2 provides the full argument.
...
\item \textbf{(F7)} Dirac neutrinos with standard electroweak couplings (changes $C_{\mathrm{total}}$ from 61 to 64, predicting $\Omega_\Lambda = 42/64 = 0.656$---excluded at ${>}\,5\sigma$ by Planck~\cite{Planck2020}).  The 45-fermion vs.\ 48-fermion discrimination is the sharpest single-observable test of $T_{\mathrm{field}}$; see Formal Supplement~\cite{BrookeP2Supplement}, \S8.
...
$L_{\mathrm{Cauchy}}$ & Cost Function Uniqueness & A1, $L_{\mathrm{loc}}$, $L_{\mathrm{cost}}$ & \texttt{check\_L\_Cauchy}; P2S \S1 \\
...
$L_{\mathrm{AF}}$ & Asymptotic Freedom & $T_3$, $\dim(\mathrm{adj}) > 0$ & \texttt{check\_T\_conf}; P2S \S4 \\
$T_{\mathrm{conf}}$ & Confinement Mechanism & $L_{\mathrm{AF}}$, A1, $L_{\varepsilon^*}$ & \texttt{check\_T\_conf}; P2S \S4 \\
...
$L_{\mathrm{gauge,uniq}}$ & Gauge Template Uniq. & Thm R, $L_{\mathrm{Cauchy}}$, Lie classif. & \texttt{check\_L\_gauge}; P2S \S II.1 \\
...
$L_{\mathrm{count}}$ & Capacity Counting & $L_{\varepsilon^*}$, $T_\kappa$, $T_3$, $T_{\mathrm{field}}$ & \texttt{check\_L\_count}; P2S \S III.1 \\
...
$L_{\mathrm{sat,part}}$ & Saturation Indep. & $L_{\mathrm{equip}}$, $L_{\mathrm{count}}$; P1S: $\delta_{\mathrm{crit}}$ & \texttt{check\_L\_sat}; P2S \S III.4 \\
...
\midrule
\multicolumn{3}{l}{\textit{From Paper 1~\cite{BrookePaper1} and Formal Supplement~\cite{BrookeSupplement} (used throughout)}} \\[2pt]
...
$L_{\mathrm{loc}}$ & Locality lemma & P1, \S4.2; FS, $T_{\mathrm{loc}}$ \\
$L_{\mathrm{irr}}$ & Irreversibility lemma & P1, \S4.5 \\
$L_\Delta$ & Superadditivity ($\Delta > 0$ at shared interfaces) & FS, \S3 \\
$L_\Pi$ & Codespace rotation (joint defender rotated into pool) & FS, \S4 \\
$T_{\mathrm{alg}}$ & Noncommutativity of enforcement algebra & FS, \S4 \\
$T_{\mathrm{sector}}$ & Finite orthogonal sector form & FS, \S3 \\
$T_M$ & Interface monogamy & P1, \S5.10; FS, $T_M$ \\
$T_3$ & Gauge structure (Skolem--Noether) & P1, \S5.7 \\
$\varepsilon^*$ & Minimum enforcement cost & P1, \S4.1; FS, $L_{\varepsilon^*}$ \\
$\kappa$ & Directed multiplier ($= 2$) & P1, \S5.11 \\[4pt]
\multicolumn{3}{l}{\textit{Introduced in this paper (proofs: Formal Supplement~\cite{BrookeP2Supplement})}} \\[2pt]
...
\bibitem{BrookeSupplement} E.~S.~Brooke, ``Formal Supplement: End-to-End Derivation from Finite Enforcement Capacity to Quantum Structure,'' companion to~\cite{BrookePaper1}.  Zenodo: 10.5281/zenodo.XXXXXXX.

\bibitem{BrookeP2Supplement} E.~S.~Brooke, ``Formal Supplement: The Structure of Admissible Physics---End-to-End Derivations,'' companion to this paper, 2026.  Self-contained proofs of $L_{\mathrm{Cauchy}}$, gauge identification, $T_{7B}$, $L_{\mathrm{irr,uniform}}$, $B1'$, $L_{\mathrm{gauge,unique}}$, $T_{\mathrm{field}}$, $L_{\mathrm{hypercharge}}$, $L_{\mathrm{count}}$, $P_{\mathrm{exhaust}}$, $L_{\mathrm{equip}}$, $L_{\mathrm{sat,part}}$, spacetime derivation chain ($\Delta_{\mathrm{sig}}$, $T_8$, $T_{9,\mathrm{grav}}$).  All imported Paper~1 results re-proved in Appendix~C.
