...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\itshape Paper 2 Formal Supplement}
...
\begin{center}
{\LARGE\bfseries Formal Supplement:\\[4pt]
...
\begin{abstract}
\noindent
This supplement provides self-contained proofs for every theorem stated in Paper~2 whose derivation depends on material from companion papers or requires detail beyond what the main text presents.  The goal is that a reader with access to Paper~1, its Formal Supplement, and this document can verify the structural chain from A1 (finite enforcement capacity) to the Standard Model gauge group and fermion content without consulting any other paper in the series.  The cosmological density fractions are derived conditionally: the structural partition $42 + 3 + 16 = 61$ is proved here; its identification with the Friedmann-equation density fractions is conditional on the horizon identification established in Paper~6.  The one-loop $\beta$-function sign is verified here using an imported formula; its derivation from capacity arguments is in Paper~4A.
...
All upstream results (A1, MD, BW, SP, K3, FD1--FD6, and the full Paper~1 derivation chain) are imported from the Paper~1 Formal Supplement and are restated in the glossary below.  Appendix~C provides complete, self-contained proofs of all ten imported results, so that no external document is needed to verify any theorem in this supplement.  No result depends on Papers~3--7; forward references to those papers appear only as scope pointers and are flagged as such.
\end{abstract}
...
\textbf{Imported from Paper~1 Supplement (proved there; re-proved in Appendix~C for self-containment).}
\begin{enumerate}[nosep,label=\arabic*.,leftmargin=2em]
  \item \textbf{A1}: enforcement capacity is finite and positive.
  \item \textbf{MD}: uniform positive cost per unit of enforcement complexity.
  \item \textbf{BW}: the cost spectrum witnesses a budget-window triple.
...
\begin{proposition}[Rational extension of $F$]\label{prop:rational_ext}
If C1 is extended to $\mathbb{Q}_{>0}$ (the positive rationals) ---
i.e., $F(a+b) = F(a) + F(b)$ for all $a, b \in \mathbb{Q}_{>0}$ ---
then $F(p/q) = p/q$ for all $p, q \in \mathbb{N}$.
\end{proposition}

\begin{proof}
For any $q \in \mathbb{N}$: $q \cdot F(p/q) = F(p/q + \cdots + p/q)
= F(q \cdot p/q) = F(p) = p$.  Therefore $F(p/q) = p/q$.
\end{proof}
...
They are not consequences of A1 (finite capacity) alone; they
require the full operational semantics established in the Paper~1
Supplement.  The precise dependency is:
\begin{enumerate}[label=(\roman*),nosep]
  ...
  enforcement semantics}, not a bare consequence of finiteness.
  \item \textbf{CM} requires: A1 $+$ MD (enforcement isotropy, via
  $L_{\varepsilon^*}$) $+$ the generator--channel bridge
  (Lemma~\ref{lem:gen_channel}).
  CM is the statement that redundant generators --- those that
  consume capacity without refining the distinction structure ---
  are inadmissible.  It requires the cost floor $\varepsilon^* > 0$
  from MD; without MD, one cannot exclude generators with
...
\textit{CM from the generator--channel bridge and cost floor.}\quad
CM requires two premises beyond A1:
\begin{enumerate}[label=(\alph*),nosep]
...
\end{enumerate}
Given (a) and (b): a generator whose removal preserves injectivity
of the label map $r_i \mapsto [r_i]_G$ --- i.e., a generator that
does not refine any multiplet distinction --- consumes
$\varepsilon^* > 0$ capacity with zero enforcement gain.  Under
finite total capacity $C_\Gamma < \infty$ (A1), every such generator
reduces the capacity available for enforcement of actual physical
distinctions without compensation.  Therefore it is excluded: the
admissible gauge structure contains no redundant generators.
\end{proof}
...
\end{tabular}
\end{center}
\noindent The only external input beyond A1 is the regularity
condition MD (enforcement isotropy), which enters through
$L_{\varepsilon^*}$.  FD3 is a foundational definition (anchor-set
locality), not an empirical assumption.  The remaining premises are
mathematical consequences of A1 and the upstream chain.

The correct characterization is therefore: \emph{EC follows from A1
alone (via the derived structural separation); CM follows from A1 $+$
MD (via the derived cost floor and generator--channel bridge).}
Neither imports a modeling assumption beyond the framework's stated
axioms.  The claim ``EC and CM follow from A1'' in earlier versions
of this text was an overclaim to the extent that CM requires MD; this
is now corrected.  The claim is \emph{not} an overclaim in the sense
that some ``additional reasonable assumption'' is smuggled in: every
step traces to either A1 or the regularity input MD, both of which
are stated in the Paper~1 axiom inventory.

\smallskip\noindent\textit{CM is not a selection principle.}\quad
...
Lemma~\ref{lem:EC_CM_from_A1} shows it is a \emph{theorem} of A1 +
MD: under finite capacity with a positive cost floor, a redundant
generator is not merely inelegant but \emph{inadmissible} because it
wastes a positive amount of a finite resource.  The generator--channel
bridge (Lemma~\ref{lem:gen_channel}) converts this from a heuristic
(``generators should be minimal'') to a quantitative statement
(``each redundant generator wastes exactly $\varepsilon^*$ capacity'').
The gauge-structure analogue is the
minimum-description-length principle: CM selects the lossless encoding
of multiplet labels with the smallest channel count.
...
\begin{enumerate}[label=(H\arabic*),nosep]
  ...
  preserving EC), both derived from A1 + MD
  (Lemma~\ref{lem:EC_CM_from_A1}).
\end{enumerate}
...
selects a unique direction in $T_{\mathrm{rel}}$.}\quad
...
Therefore the anomaly system selects a \emph{unique direction}
...
The map $J^{\otimes k}$ provides a $G$-equivariant identification between
the singlet subspace $(V^{\otimes k})^G$ and the conjugate singlet subspace
$(V^{*\otimes k})^G$.  (For real type, this is direct; for pseudoreal type,
$J^{\otimes k}$ with $k$ odd maps $V^{\otimes k} \to V^{\otimes k}$ with
$J^{2k} = (-1)^k \cdot \mathrm{id}$, but the action on gauge-invariant
tensors still identifies $B$ with a tensor in the conjugate sector.)
...
\smallskip\noindent The counts are exact and reproduced by
...
\noindent\textit{Fermion types} (45): each of the 5 multiplet types
$\{Q, u^c, d^c, L, e^c\}$ per generation is a distinct enforcement
role.  Each carries a unique set of gauge quantum numbers.  The
multiplicity within each multiplet (e.g., 3 colors of a quark) counts
internal structure, not independent enforcement roles; see
$L_{\mathrm{vac}}$ (Paper~2, \S7.1) for the vacuum-sector assignment
of internal-structure channels.
...
\smallskip\noindent The APF's named assumption count (A1, MD, BW, SP,
K3, FD1--FD6) is numerically comparable to Hardy's 5 or CDP's 6,
...
E.\,S.~Brooke, ``Formal Supplement: End-to-End Derivation from Finite
