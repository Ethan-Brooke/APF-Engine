...
per interaction.
  - Non-abelian (rank 2, m > 0): the adjoint structure resolves the d_eff microstates WITHIN each
    ...
    (L_coupling_capacity_id). At the balanced crossing it pools C_total/6 modes:
        1/alpha_cross = (C_total/6) * ln(d_eff) = S_dS/6 = 47.02.   [resolves the LOG-COUNT]
    (S_dS = C_total ln d_eff = 282.12 is the ledger LOG-COUNT, ln of the de Sitter
     entropy 102^61; legacy symbol, see THE DICTIONARY in check_T_deSitter_entropy.)
...
def check_T_abelian_coupling_fixed_by_rank1_capacity_count_P():
    ...
    return _result(
        name=("T_abelian_coupling_fixed_by_rank1_capacity_count: the absolute U(1)_Y coupling is fixed "
              ...
              "[P_structural_reading] (2026-06-27): gauge support-exhaustiveness OPEN, alpha_s-corroborated (0.11 sigma); inherits the T20 "
