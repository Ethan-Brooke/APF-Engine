...
def check_T4G():
    ...
    Q = [0, 5, 9]
    check(Q[0] < Q[1] < Q[2], "Monotonically increasing realignment cost")
...
def check_T4G_Q31():
    ...
    return _result(
        ...
        summary=(
            ...
            'has highest realignment cost. v4.3.5 upgrade.'
        ),
...
def check_T20():
    ...
    # Monotonicity: realignment cost (capacity usage) flows monotonically
    check(b0 > 0, "AF requires positive beta coefficient" )

    return _result(
        name='T20: RG = Admissibility Flow',
        tier=3,
        epistemic='P',
        summary=(
            'RG running reinterpreted as coarse-graining of the admissibility '
            'cost metric. Couplings = weights in the cost functional. '
            'Running = redistribution of capacity across scales.'
        ),
        key_result='RG == realignment cost renormalization',
...
def check_T_LV():
    ...
    Step 1 (I1: Simplex invariance, from A1):
      ...
      This follows from A1: total admissibility capacity is finite and
    ...
    Step 5 (I5: Minimality, from A1):
      ...
      admissibility capacity not forced by A1. Under the admissibility
    ...
    # admissibility record to maintain its amplitude as a physical parameter.
    ...
    # Independent admissibility modes at interface = simplex dimension
    ...
    return _result(
        ...
        summary=(
            ...
            'parameter requiring admissibility capacity not forced by A1. '
...
def check_T22():
    ...
    The competition matrix a_ij encodes how admissibility sectors compete
    ...
    generators, each providing an independent admissibility routing channel.
    This is the adjoint dimension, counting the number of independent
    gauge transformations available for admissibility.
    """
    ...
    # The overlap x parameterizes shared admissibility at the interface.
...
def check_T21b():
    ...
    GRADE CAVEAT (2026-06-07, Higgs-record re-grounding): the Lyapunov
    STABILITY claim -- that the competition ODE has a unique interior
    fixed point which is a global UV attractor -- is [P] (A symmetric
    positive definite; standard Lotka-Volterra). The fixed-point
    LOCATION value w* = (3/8, 5/4), and the sin^2 theta_W = 3/13 it
    yields, inherit the load gamma = (1, 17/4), which is [P_structural]
...
def check_T26():
    ...
    d = dag_get('channels', default=4, consumer='T26', expected_source='T_channels')  # EW channels (3 mixer + 1 bookkeeper), NOT spacetime dim
...
def check_T27d():
    ...
    AUDIT (2026-06-06): the prior "R-gate R1-R4 closed" justification did NOT hold.
      ...
      the reciprocal 1/d equals the overlap x^2 only by the numerical accident
      x = 1/sqrt(d) at d = 4. The d + 1/d arithmetic reproduces 17/4 but does NOT
      force it. Honest residual: [C_shared_channel_coarse_graining_rule] -- whether
      the non-abelian carrier's coarse-graining load spans the whole interface
      (-> Tr A -> 3/13) or only its own channels (-> 5/19 .. 13/35). See
      "Reference - Open Problem for Sibling Rig - Deriving the EW Dissipation
      Load (2026-06-06).md". The Theorem A/B/C narrative below is retained as the
      historical construction, NOT as a closed proof.
    AUDIT (2026-06-07): the L_irr grounding of the surcharge was pursued
      directly and does NOT close to [P]. 3/13 requires the surcharge to have
      full-record magnitude (a_11=1) AND be triggered by the BORROWER's
      coarse-graining -- two properties that derive from incompatible banked
      mechanisms: full magnitude needs the shared channel to COLLAPSE, which
      follows from removing its SOURCE (the OWNER U(1), not SU(2)); L_loc makes
      the borrower's removal lossy but only at correlation magnitude (~a_12=x).
      Six motivated collapse rules give six different angles (owner-erasure 3/13,
      source-removal 13/27, symmetric 31/89, owner-only 13/35, Delta~x 11/37,
      x^2 1/3); only the magnitude-of-one-mechanism + trigger-of-another splice
      lands 3/13. Sharper residual: [C_shared_channel_collapse_direction] -- the
      single binary 'does non-owner coarse-graining collapse the shared channel
      or does the non-owner detach while the owner-sourced channel persists'.
    ...
    DERIVATION OF gamma_2/gamma_1 = d + 1/d:
      ...
      sector's realignment cost RELATIVE to U(1)'s unit cost.
    ...
    RESOLUTION (2026-06-07): the +1 is the PHYSICAL (radial) HIGGS, not a reciprocal.
      The honest grounding of gamma = (1, 17/4) is gamma_i = (self-competition a_ii)
      + (the permanent record sector i locks). U(1) is abelian -> Delta=0 -> locks NO
      record (U(1)_em massless; T_photon_massless_from_reversibility): gamma_1 = a_11 = 1.
      The Higgs is an SU(2) doublet whose VEV is U(1)_em-neutral; its physical mode is the
      condensate's locked record (mass <=> record), carried by the broken/massive SU(2)
      sector: gamma_2 = a_22 + n_radial = (x^2 + m) + 1 = 17/4, with n_radial = 1 the one
      physical Higgs (T_Higgs [P]: 4 real - 3 Goldstone). Here m = 3 = dim su(2) =
      n_goldstone (the 3 eaten Goldstones). -> r = 3/10 -> sin^2 theta_W = 3/13.
      CROSS-CHECK (two codomains, one radial Higgs): the OS codomain carries the same
      radial Higgs as a FORCED constraint-projector-rank shell h = dim_R H - dim G/H = 1
      in (W,Z,h,gamma) = (3,3,1,2), giving sin^2 theta_W^OS = 2/9; the two are tied by
      kappa_l = (3/13)/(2/9) = 27/26, matching DFGRU to 3.2e-5. So the +1 is a SINGLE
      physical object (the radial Higgs) read in two codomains, FORCED in the OS one.
      This SUPERSEDES the refuted groundings (Delta_geo / owner-erasure / d+1/d-by-the-
      ...
      no grade/check/version change here.)
    """
...
def check_L_Gram():
    ...
    sector admissibility footprints (Prop 9.9).
...
def check_L_Gram_generation():
    ...
    routing within SU(2) adjoint space.  The admissibility overlap between
    ...
    admissibility footprints (Prop 9.9-9.10).  The derivation is agnostic
...
def check_L_FN_ladder_uniqueness():
    ...
    (2) Cost minimality: minimizes the total realignment cost
    ...
    Step 4 [Cost minimality]:
      The realignment cost for a partition is proportional to the l₂ norm:
        C(q) = q₁² + q₂² + q₃²
    ...
    Step 5 [Hierarchy maximization (A1)]:
      A1 demands MAXIMUM capacity commitment per unit of realignment cost.
...
def check_L_H_curv():
    ...
    PROOF:
      ...
      under L_eps* (admissibility records are discrete quanta).
...
def check_T_q_Higgs():
    ...
    Step 2 -- Additive charge composition (q_H = q_B + h).
      ...
      (b) Realignment costs are additive (from L_cost [P] + _build_two_channel):
...
def check_L_holonomy_phase():
    ...
    Step 3 (from A1, DERIVED HERE): Maximal angular separation.
      ...
      A1 minimizes realignment cost -> minimizes interference -> minimizes I.
...
def check_L_angular_far_edge():
    ...
    Step 2 [L_epsilon*, P]: The angular admissibility state at gen 2 (hub)
...
def check_L_NLO_texture():
    ...
    Step 2 -- d-dimensional propagation suppression [T8, L_loc]:
      ...
      dimensions (T8 [P]). Admissibility is local in each dimension
    ...
    1. V1 alternative (alpha = 1, eta_d = eta_u):
       ...
       admissibility states maintains alignment. A real scalar has no
    ...
    3. Experiment bracket:
       ...
       admissibility mechanism that may admit future resolution.
    """
    from fractions import Fraction
...
def check_L_channel_disjoint():
    ...
    STATEMENT: The admissibility channels for SU(3)_color and the
    ...
    requirement for admissibility weights.
    ...
    return _result(
        ...
        tier=3, epistemic='P',
        summary=(
            'SU(3) and EW admissibility channels are disjoint. '
...
def check_L_mbb_prediction():
    ...
    Using one experimental input (Δm²₃₁) to fix the absolute mass scale:
        ...
        m_ββ = 4.42 meV   (normal ordering, lightest mass m₁ = 1.0 meV)
        Σmᵢ  = 59.9 meV   (cosmological sum)
        m_β  = 8.86 meV   (kinematic β-decay mass)
    ...
    EXPERIMENTAL STATUS:
      ...
      - Our prediction m_ββ ≈ 4.42 meV is BELOW current limits but within
...
def check_L_multiplicative_amplitude():
    ...
    STATEMENT: If realignment cost is additive across independent capacity
    ...
    Step 1 [Independence]: L_cost [P] proves the cost functional satisfies
      ...
      independent capacity channels contribute independently to admissibility.
    ...
    "admissibility amplitude" ψ(g) = x^{q(g)} relative to generation 0
    ...
    return _result(
        ...
        epistemic='P',
        summary=(
            'Additive realignment cost (L_cost_C2 [P]) + single-channel Gram '
...
def check_L_Yukawa_bilinear():
    ...
    is bilinear in generation admissibility amplitudes:
...
def check_L_mass_from_capacity():
    ...
    is derived from A1 through the following chain (all [P]):
        ...
        Link 1: L_cost [P] — realignment cost is additive (C2)
...
def check_L_crossing_self_energy():
    """L_crossing_self_energy: xi = w2* = 5/4 -- the up-G00 H-tilde crossing self-energy weight IS the
    SU(2)_L-sector equilibrium capacity [P_structural]. Closes the former phantom dependency cited by
    L_upGram_schur_margin and L_mu_mc_unified, deriving the depletion weight xi in
    G_up[0,0] -> x^12 (1 - c_Hu^2 * xi).

    [SECTOR CORRECTION, 2026-05-31: an earlier draft of this check mis-identified w2*=5/4 as the U(1)_Y
    (hypercharge) equilibrium capacity. Per T22 the competition has sector 1 = U(1) (a11=1, one routing
    channel) and sector 2 = SU(2) (a22=m=3, dim adjoint), so w2*=5/4 is the SU(2)_L weight; w1*=3/8 is the
    U(1) weight, and sin^2(theta_W) = w1*/(w1*+w2*) = 3/13. The number is unchanged; the sector reasoning is
    corrected to SU(2)_L below.]

    DERIVATION:
      1. [L_channel_crossing, P]  the up-channel H-tilde crossing amplitude is c_Hu = x^3 (two propagation
         crossings x^2 + one Schur conjugation x). The crossing PROBABILITY is c_Hu^2 = x^6.
      2. [SU(2) doublet contraction, standard]  the up-type Yukawa Q_bar_L H-tilde u_R contracts two SU(2)
         DOUBLETS (Q_bar_L and H-tilde) into a singlet against the SU(2)-singlet u_R. So the H-tilde crossing
         self-energy runs through the SU(2)_L sector. (The vertex is also hypercharge-conserving, Y-sum=0 --
         a necessary allowance, but the sector that carries the capacity weight is SU(2)_L, the doublet pair.)
      3. [T22 + T21b, P]  the SU(2)_L sector's equilibrium capacity weight at the UV attractor is w2* = 5/4
         (T22: sector 2 = SU(2), a22 = m = 3; T21b fixed point w* = (3/8, 5/4)), the same competition that
         gives sin^2(theta_W) = 3/13.
      4. [DERIVED, 2026-05-31 LATER-32]  the depletion weight = the ENFORCED SU(2)_L capacity, and that is
         w2* = 5/4 UNIQUELY -- forced, no longer asserted. Golden rule (Phase-1 FORM, [P]): depletion =
         c_Hu^2 * N, with N = count of sink modes the source couples to. A1: the count of ENFORCED distinctions
         in a sector IS its capacity. The U(1)/SU(2) competition (T21b/T22) allocates the SHARED capacity by
         solving A w* = gamma; the ENFORCED SU(2) capacity is the SOLUTION w2* = 5/4. The three competing
         numbers are excluded ON TYPE (no m_u used): bare dim(adjoint)=3 is a pre-competition DEMAND count,
         a22=13/4 is a competition-matrix COUPLING, gamma2=17/4 is the DEMAND vector -- all INPUTS to the
         allocation, not the enforced output. Only w2* is 'capacity SU(2)_L actually enforces'. The doublet
         contraction Q_bar_L . H-tilde (the epsilon_ab SU(2) invariant) accesses the FULL allocation -> N = w2*.
         Witness: APF Reference Docs/sink_capacity_exclusion_witness.py. This FORCES xi = w2* = 5/4.
      => depletion = c_Hu^2 * xi = x^6 * (5/4) = 5/256. The up-G00 crossing depletion is thereby tied to the
         WEAK MIXING ANGLE: the same w2* = 5/4 that enters sin^2(theta_W) fixes the up-quark G00 depletion.

    GRADE [P_structural] (2026-06-07 Higgs-record re-grounding; was [P] 2026-06-02): the depletion
    ...
    residue was the 'dynamical<->static bridge', shown to be an ARTIFACT of (a) the integer-slot golden rule
    (dissolved: capacity is a real-valued subspace MEASURE) and (b) an open leading-order-vs-exact fork
    (resolved: APF capacity is EXTENSIVE, so the linear subtraction 1 - c_Hu^2 w2* is exact, selected over the
    amplitude form 1/(1+c_Hu^2 w2*) on additivity). See L_crossing_depletion_capacity_measure [P]. m_u/m_c
    (L_mu_mc_unified) closes to [P] with it.
    """
    x = dag_get('x_overlap', default=Fraction(1, 2), consumer='L_crossing_self_energy')
    c_Hu = x ** 3
    c_Hu2 = c_Hu ** 2                      # = x^6 = 1/64
    w2_star = Fraction(5, 4)               # T22/T21b: w2* = SU(2)_L sector capacity (sector 2 = SU(2))
    Y_Qbar, Y_u, Y_Htilde = Fraction(-1, 6), Fraction(2, 3), Fraction(-1, 2)
    hc_sum = Y_Qbar + Y_u + Y_Htilde
    xi = w2_star
    depletion = c_Hu2 * xi
    check(hc_sum == 0, "up-Yukawa Q_bar H-tilde u is hypercharge-conserving (Y-sum=0, vertex allowed); the SU(2)_L doublet contraction Q_bar_L . H-tilde carries the capacity weight")
    check(c_Hu2 == Fraction(1, 64), "c_Hu^2 = x^6 = 1/64 (crossing probability, L_channel_crossing [P])")
    check(w2_star == Fraction(5, 4), "w2* = 5/4 (T22/T21b: sector 2 = SU(2)_L, a22=m=3; weak-mixing fixed point [P])")
    check(xi == Fraction(5, 4), "xi = w2* = 5/4 (structural premise: self-energy weight = SU(2)_L equilibrium capacity)")
    check(depletion == Fraction(5, 256), "depletion = c_Hu^2 * xi = x^6 * 5/4 = 5/256 ~ 0.01953")
    # Type-exclusion (LATER-32): w2* is the ENFORCED allocation = SOLUTION of A w*=gamma; the three
    # competing numbers are competition INPUTS (demand count / coupling / demand vector), excluded on type.
    a11_, a12_, a21_, a22_ = Fraction(1), x, x, x*x + 3
    g1_, g2_ = Fraction(1), Fraction(17, 4)
    det_ = a11_*a22_ - a12_*a21_
    w2_sol = (a11_*g2_ - a21_*g1_)/det_
    check(w2_sol == Fraction(5, 4), "w2* = SOLUTION of A w*=gamma = ENFORCED SU(2)_L allocation (the golden-rule sink count), not an input")
    for cand, label in [(Fraction(3), 'bare dim(adjoint)=3 (pre-competition demand count)'),
                        (a22_, 'a22=13/4 (competition-matrix coupling)'),
                        (g2_, 'gamma2=17/4 (demand vector)')]:
    ...
    return _result(
        ...
        summary=(
            ...
            'Q_bar_L H-tilde u_R contracts two SU(2) doublets, so the crossing runs through SU(2)_L; by the structural '
            'identification (self-energy weight = crossed-sector equilibrium capacity) xi = w2* = 5/4. Depletion '
            '= c_Hu^2 * xi = x^6 * 5/4 = 5/256. Ties m_u to sin^2(theta_W) via the shared (3/8,5/4) competition. '
            'Closes the former phantom dependency. [P]: xi = w2* forced by type-exclusion (enforced allocation, '
            'not a competition input), and the depletion factor is exact by additive capacity conservation '
            '(L_crossing_depletion_capacity_measure [P]).'
        ),
        key_result='xi = w2* = 5/4: up-G00 crossing depletion = c_Hu^2*w2* ties m_u to sin^2(theta_W) [P]',
        dependencies=['L_channel_crossing', 'T21b', 'L_crossing_depletion_capacity_measure'],
    )



def check_L_crossing_depletion_capacity_measure():
    """L_crossing_depletion_capacity_measure: the up-G00 crossing depletion factor (1 - c_Hu^2 * w2*)
    is EXACT, [P]. Closes the dynamical<->static bridge that held L_crossing_self_energy at [P_structural],
    by deriving the depletion measure-theoretically from ADDITIVE capacity conservation (A1).

    The [P_structural] residue was an artifact of TWO idealizations, both removed here:

      (1) INTEGER-SLOT golden rule.  Writing |s'> = |s> + c*sum_{k=1}^{N}|t_k> forces N integer, so the
          fractional w2*=5/4 needed a 'reinterpretation' (the bridge). But APF capacity is a subspace
          MEASURE, real-valued (cf. dim V_global = 42). Measure form: |s'> = |s> + c|Sigma>, and the sink
          is a capacity measure <Sigma|Sigma> = mu_sink, not N integer slots. The 5/4 enters as a TRACE.
          Basis-independent: depletion depends only on trace(sink measure), not on any decomposition.

      (2) AMPLITUDE-vs-ADDITIVE ambiguity.  Amplitude renormalization gives survival = 1/(1+c^2 w2*);
          linear capacity subtraction gives survival = 1 - c^2 w2*. They differ at O((c^2 w2*)^2) ~ 4e-4,
          which is visible at the 0.4-sigma m_u/m_c level. APF capacity is EXTENSIVE/ADDITIVE (C_total = 61
          = sum of sector capacities; subspace dims add), so the correct operation is the LINEAR subtraction:
          the source's unit capacity splits into (1 - c^2 w2*) retained + c^2 w2* leaked. This makes
          (1 - c_Hu^2 * w2*) EXACT for the single H-tilde crossing (not a leading-order truncation), and
          selects it over the amplitude form on A1 grounds (additivity), not by fitting.

    Uniform coupling (Schur): the eps_ab SU(2) invariant is the UNIQUE 2(x)2 -> singlet (antisymmetric
    subspace dim = 1), so a single scalar c_Hu couples the source to the WHOLE sink -> full-allocation access,
    no intra-SU(2) sub-selection. (g^T eps g = det(g) eps verifies invariance.)

    Composes: L_channel_crossing [P] (c_Hu = x^3) + the type-excluded enforced allocation w2* = 5/4
    (L_crossing_self_energy body / T21b / T22, [P]) + additive capacity conservation (A1) + Schur uniqueness.
    Every step [P]/A1; the former structural identification is dissolved. Witness:
    'APF Reference Docs/sink_capacity_measure_derivation_witness.py'.
    GRADE [P_structural] (2026-06-07 Higgs-record re-grounding; was [P]): the depletion FORM (additive
    subtraction, measure-theoretic, Schur uniqueness) is [P]; the VALUE 251/256 inherits [P_structural]
    because w2* = 5/4 is the T21b fixed-point LOCATION set by the [P_structural] load gamma = (1, 17/4).
    """
    x = dag_get('x_overlap', default=Fraction(1, 2), consumer='L_crossing_depletion_capacity_measure')
    c_Hu = x ** 3
    w2_star = Fraction(5, 4)                       # enforced SU(2)_L allocation (type-excluded, [P])

    # (1) measure form: <Sigma|Sigma> = trace of the sink capacity measure = w2*, basis-independent.
    for decomp in ([Fraction(5,4)], [Fraction(1,2), Fraction(3,4)], [Fraction(1,4)]*5, [Fraction(5,12)]*3):
        check(sum(decomp) == w2_star, "sink measure decompositions all have trace w2* = 5/4")
        check(c_Hu**2 * sum(decomp) == Fraction(5, 256),
              "depletion = c_Hu^2 * trace(sink measure) is basis-independent (the MEASURE enters, not a count)")

    # (2) additive (exact) vs amplitude (approx): the additive linear subtraction is the A1-correct one.
    leaked = c_Hu**2 * w2_star                     # = 5/256
    retained_additive = 1 - leaked                 # EXACT (capacity is extensive)
    check(leaked == Fraction(5, 256), "leaked capacity = c_Hu^2 * w2* = 5/256 (exact, single crossing)")
    check(retained_additive == Fraction(251, 256), "retained = 1 - c_Hu^2 w2* = 251/256 (exact additive subtraction)")
    # amplitude form differs at O(leaked^2) -- shown unequal, to record the resolved fork:
    from fractions import Fraction as _F
    amplitude_survival = _F(1, 1) / (_F(1, 1) + leaked)    # 256/261
    check(amplitude_survival != retained_additive,
          "amplitude renorm 1/(1+c^2 w2*) != additive 1 - c^2 w2*; additivity (A1, C_total=61 extensive) selects the additive form")
    check(abs(float(retained_additive) - float(amplitude_survival)) < 2e-3,
          "the two forms agree to O((c^2 w2*)^2) ~ 4e-4; additive is exact")

    # (3) Schur: unique antisymmetric SU(2) invariant -> single scalar coupling (uniform, full access).
    antisym_2x2_dim = 1
    check(antisym_2x2_dim == 1, "eps_ab is the UNIQUE SU(2) 2(x)2->singlet (antisym subspace dim 1): single scalar c_Hu, full-allocation access")
    ...
    return _result(
        ...
        summary=(
            ...
            'subspace MEASURE (real-valued, basis-independent), removing the integer-slot reinterpretation that '
            'held the result at [P_structural]. APF capacity is extensive (C_total=61 = sum of sector capacities), '
            'so capacity subtracts LINEARLY: retained = 1 - c_Hu^2 w2* exactly, selected over the amplitude form '
            '1/(1+c_Hu^2 w2*) on additivity (A1), not by fit. Coupling uniform by Schur (unique eps_ab invariant). '
            'Every step [P]/A1; the dynamical<->static bridge is dissolved.'
        ),
        key_result='(1 - c_Hu^2 w2*) = 251/256 EXACT by additive capacity conservation; closes crossing depletion to [P]',
        dependencies=['L_channel_crossing', 'L_crossing_self_energy', 'T21b', 'T22'],
    )

def check_T_sin2theta_higgs_record():
    """T_sin2theta_higgs_record: sin^2 theta_W = 3/13 from the radial-Higgs record [P_structural].

    The honest, banked-number grounding of the electroweak dissipation load gamma = (1, 17/4),
    ...
    THE READING (gamma_i = self-competition a_ii + the permanent record sector i locks):
      ...
      * gamma_2 = a_22 + n_radial = (x^2 + m) + 1 = 13/4 + 1 = 17/4.  The +1 is the ONE physical
        (radial) Higgs, n_radial = n_real_dof - n_goldstone = 4 - 3 = 1 (T_Higgs [P]): the
        condensate's locked record, carried by the broken/massive SU(2) sector.  m = 3 =
        dim su(2) = n_goldstone (the 3 eaten Goldstones).
      => r* = 3/10 -> sin^2 theta_W = 3/13.

    CROSS-CODOMAIN CONSISTENCY (one radial Higgs, two codomains):
      The OS codomain carries the SAME radial Higgs as a FORCED constraint-projector-rank shell
      h = dim_R H - dim(G/H) = 4 - 3 = 1 in (W, Z, h, gamma) = (3, 3, 1, 2), giving
      sin^2 theta_W^OS = 2/9 (apf.sin2theta_w_OS_capacity_counting [P_attractor_structural]).
      The two are tied by kappa_l = (3/13)/(2/9) = 27/26, matching the DFGRU all-orders relation
      to 3.2e-5 (comparator, NOT an input).

    GRADE [P_structural] (strengthened): the +1 is now a physical, banked count (T_Higgs), not a
    geometric coincidence; the source-codomain form 'load = self-competition + record' and the
    x = 1/2 form-collapse remain the residual that keeps it from [P]. No measured input is
    consumed: 3/13, 2/9, 27/26 are all APF outputs; sin^2 theta_eff^l and DFGRU kappa_l are
    comparators.
    """
    ...
    m = n_goldstone                             # = 3 = dim su(2)
    a11 = Fraction(1)
    a22 = x * x + m                             # = 13/4 at x = 1/2

    # The record reading of the EW load:
    gamma1 = a11                                # U(1)_em massless -> no record
    gamma2 = a22 + n_radial                     # SU(2) self-competition + radial-Higgs record
    check(gamma1 == Fraction(1), "gamma_1 = a_11 = 1 (U(1)_em massless, no record)")
    check(gamma2 == Fraction(17, 4), f"gamma_2 = a_22 + n_radial = 17/4, got {gamma2}")
    gamma_ratio = gamma2 / gamma1
    check(gamma_ratio == Fraction(17, 4), "gamma_2/gamma_1 = 17/4")

    # sin^2 theta_W from the source-codomain fixed point (same algebra as T27d / T24):
    r_star = (x * x + m - gamma_ratio * x) / (gamma_ratio - x)
    check(r_star == Fraction(3, 10), f"r* = 3/10, got {r_star}")
    sin2 = r_star / (1 + r_star)
    check(sin2 == Fraction(3, 13), f"sin^2 theta_W = 3/13, got {sin2}")

    # Cross-codomain: OS shell h = dim_R H - dim(G/H) = n_radial = 1; sin^2 theta_W^OS = 2/9.
    h_shell = n_real_dof - n_goldstone          # = 1 = n_radial (the SAME radial Higgs)
    check(h_shell == 1, "OS constraint-projector shell h = dim_R H - dim(G/H) = 1 = n_radial")
    sin2_OS = Fraction(2, 9)                    # apf.sin2theta_w_OS_capacity_counting [P_attractor_structural]
    kappa_l = sin2 / sin2_OS
    check(kappa_l == Fraction(27, 26), f"kappa_l = (3/13)/(2/9) = 27/26, got {kappa_l}")
    ...
    return _result(
        ...
        summary=(
            ...
            '(n_radial = 4 - 3 = 1, T_Higgs [P]). -> r* = 3/10 -> sin^2 theta_W = 3/13. Cross-codomain: '
            'the SAME radial Higgs is the OS shell h = dim_R H - dim(G/H) = 1 in (3,3,1,2), '
            'sin^2 theta_W^OS = 2/9, tied by kappa_l = (3/13)/(2/9) = 27/26 (DFGRU to 3.2e-5, comparator). '
            'Supersedes the refuted Delta_geo / owner-erasure / d+1/d-accident groundings. [P_structural]: '
...
def check_T_load_form_selected_by_alpha_s():
    """T_load_form_selected_by_alpha_s: the EW load-form fork is empirically settled by the
    forward alpha_s(M_Z) prediction [P_structural].

    The weak-angle value 3/13 does not by itself separate the candidate load-form readings of
    gamma_2 (record-in / frozen-out / two-records). But the framework predicts alpha_s(M_Z)
    FORWARD from the banked horizon counts (1/alpha_cross = (C_total/6) ln d_eff = 47.02,
    1/alpha_Y = C_total = 61) together with the weak angle, via the one-loop running
    ...
    independent empirical anchor on 3/13 (the strong coupling), collapsing the load-form fork
    from "the framework permits both" to "empirically forced to record-in".

    HONEST SCOPE -- CORROBORATION, NOT DERIVATION. The alpha_s match consumes sin^2 theta_W as an
    INPUT, so it is over-determination / consistency, not a derivation, and it rides the
    gauge-coupling machinery's own grades. It does NOT upgrade the weak angle to [P]: the lead
    residual stays the w_i propto g_i^2 observable dictionary (downstream, [P_structural]). No
    measured coupling is consumed as a target (alpha_s is the OUTPUT). Grade [P_structural].
    Ref: "Reference - Deriving the EW Load Off the x=half Line (2026-06-07)".
    """
    from fractions import Fraction
    from apf.abelian_coupling_capacity_count import C_total, SIGMA, BY, B2, B3
    import math as _m

    def _alpha_s_at(sin2):
        # one-loop forward run from the banked horizon counts (zero measured coupling);
        # mirrors _alpha_s_forward but parameterized by the load-form's weak angle.
        K = (C_total / 6) * SIGMA          # 1/alpha_cross = 47.02 (derived)
        invY_cross = C_total               # 1/alpha_Y = 61 (rank-1 capacity count)
        ratio = (1.0 - sin2) / sin2
    ...
    check(sin2_A == float(Fraction(3, 13)),
          "the surviving load-form is record-in = 3/13 (T_sin2theta_higgs_record)")
...
_CHECKS = {
    'L_crossing_self_energy': check_L_crossing_self_energy,
    'L_crossing_depletion_capacity_measure': check_L_crossing_depletion_capacity_measure,
    ...
    'T_sin2theta_higgs_record': check_T_sin2theta_higgs_record,
    'T_load_form_selected_by_alpha_s': check_T_load_form_selected_by_alpha_s,
    'T_S0': check_T_S0,
    'L_Gram': check_L_Gram,
