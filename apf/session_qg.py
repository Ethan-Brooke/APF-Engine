...
def check_L_QG_P1_closure():
    ...
    M_Pl = 1.22089e19
    a_Y = 2.630; c_R = 1.968  # c_R = RETIRED tuned value (real 1/2 Tr(k+k)=21.34); route [C] 2026-05-29
    a_total_vev = a_Y + c_R
    b = 2.305
    ...
    return _result(
        name='L_QG_P1_closure: Quantum Gravity Sector — P1 §3.1 Status [RE-GRADED C]',
        tier=5, epistemic='C',
        summary=(
        ...
        key_result=(
            ...
            f'149 GeV → 124.9 GeV (κ_R dilution). v: 2% → 0.11% (threshold matching). '
            f'[C — §3.1 absolute-scale closure rests on tuned c_R=1.968 (computed κ trace=21.34); re-graded 2026-05-29]'
        ),
        dependencies=[
...
def check_L_vev_coleman_weinberg():
    ...
    g2 = 0.6517
    g1 = 0.3574
    lam = 0.1291  # tree lambda = m_H^2/(2 v^2) ~ 0.129 (NB: L_Higgs_2loop RG value is 0.125)
    ...
    return _result(
        name='L_vev_coleman_weinberg: CW Correction to Spectral VEV [RE-GRADED C]',
        tier=5, epistemic='C',
        summary=(
        ...
        key_result=(
            ...
            f'2% residual is structural, not loop-level. '
            f'[C — operates on tuned v_tree=251.1 from L_hierarchy_boson_suppression; re-graded 2026-05-29]'
        ),
        dependencies=[
...
def check_L_vev_threshold_matching():
    ...
    Step 3 [Evaluate traces at μ_R]:
        ...
        c_R = 1.968 (RETIRED tuned value; NOT the Majorana singlet trace, real 1/2 Tr(k+k)=21.34; route [C] 2026-05-29)
        a_total(μ_R) = 4.716
    ...
    C_boson = 16
    d_eff = 102
    c_R = 1.968          # RETIRED tuned value -- NOT the Majorana trace (real 1/2 Tr(k+k)=21.34); route [C] 2026-05-29
    N_c = 3
    alpha_s_MZ = 0.1179  # external input (PDG; apf_utils canonical 0.1181); NOT APF-derived
    ...
    return _result(
        name='L_vev_threshold_matching: EW VEV at ν_R Decoupling Scale [RE-GRADED C]',
        tier=3, epistemic='C',
        summary=(
        ...
        key_result=(
            ...
            f'Gap CLOSED by threshold matching at μ_R = {mu_R:.1f} GeV. '
            f'[C — uses hardcoded c_R=1.968 (computed κ trace=21.34→1309 GeV); re-graded 2026-05-29]'
        ),
        dependencies=[
