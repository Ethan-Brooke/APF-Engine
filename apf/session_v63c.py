...
def check_L_hierarchy_boson_suppression():
    ...
    v6.3c. RE-GRADED [P] -> [C] on 2026-05-29 (bank-integrity audit).
    The c_R = 1.968 used in a_total below is a HARDCODED value, inconsistent with
    the framework's own computed 1/2 Tr(kappa^T kappa) = 21.34 (the identical
    kappa_R construction in majorana.py and internalization.py, which gives the
    correct m_H). With the computed 21.34, a_total = 23.97 and this formula gives
    v_H = 1309 GeV, not 251. So P1 §2.1 is NOT closed natively by this route; the
    251 GeV agreement depends on the tuned c_R. No derivation of 1.968 exists.
    See wiki/Open Problems.md (2026-05-29 bank-integrity flag).
    ...
    c_R = 1.968       # RETIRED tuned value -- NOT 1/2 Tr(k+k) (real=21.34); route [C] 2026-05-29
    ...
    return _result(
        name='L_hierarchy_boson_suppression: EW Scale from Capacity Geometry [RE-GRADED C]',
        tier=3, epistemic='C',
        summary=(
        ...
        key_result=f'v = {v_pred:.1f} GeV via HARDCODED c_R=1.968; computed kappa trace=21.34 -> 1309 GeV. P1 §2.1 NOT closed. [C] (re-graded 2026-05-29)',
...
def check_L_hierarchy_cascade():
    ...
    return _result(
        name='L_hierarchy_cascade: Cascade Effects from EW Scale Derivation [RE-GRADED C]',
        tier=3, epistemic='C',
        summary=(
            f'σ₀ = {sigma_0:.1f} GeV from σ₀²/v² = 0.01340 × v = {v_pred} GeV. '
            f'M_R = [{M_R[0]:.1f}, {M_R[1]:.1f}, {M_R[2]:.1f}] GeV. '
            f'M_R REVERTS [P] → [P+anchor]; P1 §2.2 closure WITHDRAWN — '
            f'σ₀ and M_R inherit v=251.13 from the tuned-c_R vev (re-graded 2026-05-29).'
        ),
        key_result=f'σ₀ = {sigma_0:.1f} GeV, M_R = [{M_R[0]:.0f},{M_R[1]:.0f},{M_R[2]:.0f}] GeV — [C]; M_R reverts to [P+anchor] (rests on tuned c_R vev)',
...
def check_L_neutrino_closure():
    ...
    return _result(
        ...
        summary=(
            ...
            f'but the §1.3 *closure* claim depended on M_R [P] via the cascade, which now reverts '
            f'to [P+anchor] (tuned-c_R vev). [C] (re-graded 2026-05-29).'
        ),
...
def check_L_yD_spectral():
    ...
    return _result(
        ...
        key_result=(
            ...
            f'Absolute Δm²₃₁ uses hardcoded v=251.13/σ₀=29.07 (tuned-c_R vev); '
            f'the absolute anchor REVERTS (anchors 0→1). y_D ratio structure survives. [C] (re-graded 2026-05-29)'
        ),
        dependencies=[
