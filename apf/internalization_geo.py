...
def check_L_kolmogorov_internal():
    ...
    STATEMENT: The continuum limit of the admissibility lattice exists and
    ...
    Step 1 [A1 → finite lattice]:
      A1 states admissibility capacity C is finite. Any finite region
      contains finitely many admissibility events. At resolution scale ε,
    ...
    Step 5 [Continuum measure = admissibility field]:
      The unique limit measure μ on the continuum is the admissibility
...
def check_L_chartability():
    ...
    STATEMENT: The continuum admissibility space (from L_kolmogorov_internal)
    ...
    Step 1 [Cost function → metric]:
      L_epsilon* [P] defines the realignment cost ε between any two
    ...
    Step 2 [A1 → bounded diameter]:
      A1 (C < ∞) bounds the maximum realignment cost:
        sup_{x,y} ε(x,y) ≤ C_total · ε* = 61
      The metric space (X, ε) has bounded diameter.

    Step 3 [Delta_fbc → Lipschitz regularity]:
      Delta_fbc [P] proves the Finite Boundary Condition: admissibility
      variation |Δφ| ≤ C_max/N (Lipschitz bound). In the continuum
      limit, this becomes:
        |∂_μ φ| ≤ K   (uniform Lipschitz bound on the admissibility field)
    ...
    Step 4 [Lipschitz → C^{1,α} → C^∞ via elliptic regularity]:
      The admissibility field satisfies a second-order elliptic PDE
      (from the variation of the cost functional — the Euler-Lagrange
      equation of the admissibility action). For Lipschitz initial data:
    ...
    the APF constructs the atlas INTRINSICALLY from the admissibility
    ...
    # 3 points with realignment costs
    ...
    return _result(
        ...
        summary=(
            ...
            'from realignment cost metric + regularity bootstrap.'
        ),
        key_result=(
...
def check_L_coleman_mandula_internal():
    ...
    STATEMENT: The symmetry group of the admissibility framework
    ...
    Step 2 [L_loc → no spacetime-internal mixing]:
      L_loc [P] states: admissibility operations at spacelike-separated
