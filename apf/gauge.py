...
def check_T_particle():
    ...
    The admissibility potential V(Phi) is derived from:
    ...
    # The admissibility potential V(Phi) = epsilonPhi (eta/2epsilon)Phi^2 + epsilonPhi^2/(2(CPhi))
    ...
    return _result(
        ...
        epistemic='P',
        summary=(
            'Admissibility potential V(Phi) derived from L_epsilon* + T_M + A1. '
...
def check_T_confinement():
    ...
    Step 2 [IR capacity-saturation -- CARRIED PREMISE, not a [P] consequence of A1]:
      The mechanism fires AT IR capacity-saturation: an IR scale where the
      remaining admissibility slack drops below epsilon. Whether the IR flow
      REACHES such a saturation is NOT decided by A1. Finite capacity is agnostic
      between (a) the coupling growing without bound -> saturation -> confinement,
      and (b) the coupling approaching an IR fixed point -> conformal -> gapless
      (its soft modes sub-threshold, hence consistent with MD; audit 2026-05-31,
      'Does Finite Capacity Force the Gap'). For the PHYSICAL strong sector the
      antecedent is empirically satisfied (QCD confines at Lambda_QCD). As a
      CONTINUUM-existence question -- does the matter-free non-abelian limit have
      no nontrivial IR zero -- it is OPEN (the mass-gap half of Clay). So
      saturation is the antecedent the mechanism class is conditioned on, not a
      result derived here.
    ...
    Step 4 [Saturation premise (Step 2) + Step 3]:
      At saturation (the Step-2 antecedent), available slack < epsilon.
    ...
    HONEST LIMITATION: this check derives the MECHANISM CLASS only -- the
    CONDITIONAL "at saturation (slack < epsilon), non-singlets are inadmissible",
    which is [P] from L_epsilon* (Steps 3-4). It does NOT derive that saturation
    OCCURS. T4F establishes flavor/EW-capacity saturation (75%), NOT strong-sector
    IR saturation; and IR coupling growth (Step 1, perturbative AF) + A1 do NOT
    guarantee it -- finite capacity is agnostic between saturation and an IR fixed
    point (audit 2026-05-31). Consumers must SUPPLY the antecedent: empirically for
...
def check_T9():
    ...
    k admissibility operations in all k! orderings -> k! orthogonal record sectors.
    """
    ...
    return _result(
        ...
        epistemic='P',
        summary=(
            f'k = {k} admissibility operations -> {n_histories} inequivalent histories. '
...
def check_T_theta_QCD():
    ...
    Step 2: A definite theta requires admissibility (L_epsilon*).
    ...
    Step 4: theta != 0 strictly more expensive (A1 selects minimum cost).
      ...
      realignment cost at fixed capacity. This is theta = 0.
    ...
    # Net admissibility surplus: capacity_gain - cost
    ...
    # CKM phase realignment cost: at least epsilon
...
def check_T4E():
    ...
    C_EW = 8
    E_per_gen = 2  # realignment cost per generation
    ...
    # Realignment costs per generation (in units of epsilon)
...
def check_L_count():
    """L_count: Capacity Counting ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 1 structural admissibility channel = 1 unit.

    STATEMENT: At Bekenstein saturation, the number of independently
    enforceable capacity units equals the number of STRUCTURAL admissibility
    ...
    do not contribute independent admissibility channels.
    ...
    Step 3 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Structural vs kinematic DOF:
      A structural admissibility channel is an independently enforceable
      element of the admissibility algebra:
        (a) T3 [P]: gauge automorphisms are independent directions in Lie(G).
            ...
            Polarizations describe propagation (kinematic), not admissibility
    ...
    Step 4 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Independence (L_loc + T_M [P]):
      ...
      admissibility resources ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the counting is additive.
    ...
    Step 5 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Minimality (T_kappa + L_epsilon* [P]):
      Each structural channel is EXACTLY one distinction because:
        ...
        (b) It cannot be decomposed further without violating L_epsilon*
            (sub-channel realignment cost would be < epsilon).
    ...
    WHY NOT count polarizations/helicities:
      ...
      post-hoc fitting but a consequence of counting admissibility channels
      rather than field modes.

    FALSIFIABILITY:
      F_count_1: If any SM DOF costs != epsilon at saturation, C_total changes.
      F_count_2: If kinematic DOF carry independent realignment cost,
    ...
    # ================================================================
    # Step 3: Count structural admissibility channels
    # ================================================================
    ...
    return _result(
        ...
        epistemic='P',
        summary=(
            'Capacity units = structural admissibility channels, not field modes. '
        ...
        key_result='C_total = 61 structural admissibility channels (derived, not assumed)',
        ...
        artifacts={
            ...
            'n_gauge': 12,
            'C_total': 61,
            'counting_principle': 'structural admissibility channels',
...
def check_L_dim_angle():
    ...
    Step 4: Angular budget principle.
      ...
      Each unit of mass dimension = one admissibility channel.
...
def check_T_proton():
    ...
    Step 3 -- No admissible rerouting [P_exhaust + T_particle, P]:
      At saturation, the admissibility potential V(Phi) sits at its
    ...
    # Check: the admissibility potential forbids un-saturation
    eps = Fraction(1, 10)
    C = Fraction(1)

    def V_rational(phi):
        """Admissibility potential at rational phi."""
        if phi >= C:
    ...
    return _result(
        ...
        summary=(
            ...
            f'predicates Q1,Q2 are exact, admissibility potential '
        ...
        dependencies=[
            ...
            'T_particle',          # Admissibility potential well
...
def check_L_strong_CP_synthesis():
    ...
    (1) STRONG SECTOR: theta_QCD = 0 [T_theta_QCD, P]
      ...
      theta != 0 costs admissibility (L_epsilon*) with zero gain.
...
def check_T_vacuum_stability():
    ...
    no deeper vacuum to tunnel to. The admissibility potential has a
    ...
    Step 1 -- Unique admissibility well [T_particle, P]:
      The admissibility potential V(Phi) has:
    ...
    """
    # ================================================================
    # Step 1: Unique admissibility well
    # ================================================================
    C = Fraction(1)
    eps = Fraction(1, 10)
    ...
    # In the framework, the admissibility potential replaces the
    ...
    return _result(
        ...
        epistemic='P',
        summary=(
            'EW vacuum is absolutely stable [P]. Admissibility potential '
